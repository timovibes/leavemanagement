...
const GRADES = [
  'KFS_1','KFS_2','KFS_3','KFS_4',
  'KFS_5','KFS_6','KFS_7','KFS_8',
]
...
// ── Balance Manager ──────────────────────────
function BalanceManager({ employee, leaveTypes, onClose }) {
  const [saving, setSaving] = useState(false)
  const currentYear = new Date().getFullYear()

  const handleSave = async (typeId, entitlement) => {
    setSaving(true)
    try {
      await api.post('/leaves/balances/set/', {
        employee: employee.id,
        leave_type: typeId,
        year: currentYear,
        total_entitlement: parseInt(entitlement),
      })
    ...
    <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center
      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-xl
        ...
        <div className="sticky top-0 bg-white px-5 pt-5 pb-3
                        border-b border-gray-100 flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-gray-800">Leave Balances</h3>
            <p className="text-xs text-gray-500">{employee.name} — {currentYear}</p>
          </div>
          <button onClick={onClose}><X size={18} className="text-gray-400" /></button>
        </div>
...
function LeaveTypeBalanceRow({ type, employee, year, onSave, saving }) {
  ...
  const load = async () => {
    ...
    try {
      const res = await api.get(
        `/leaves/balances/?employee=${employee.id}&leave_type=${type.id}&year=${year}`
      )
      const data = res.data?.results || res.data || []
      const bal = data[0]
      if (bal) {
        setCurrent(bal)
        setValue(bal.total_entitlement)
      }
      setLoaded(true)
    } catch {}
  }
  ...
  return (
    <div className="flex items-center gap-3" onMouseEnter={load}>
      <div className="flex-1">
        <p className="text-sm font-medium text-gray-800">{type.name}</p>
        <p className="text-xs text-gray-400">Max: {type.max_days} days</p>
        {current && (
          <p className="text-xs text-kfs-green">
            Current: {current.total_entitlement}d •
            Taken: {current.taken}d •
            Remaining: {current.remaining}d
          </p>
        )}
      </div>
      <div className="flex items-center gap-2">
        <input
          ...
          placeholder={type.max_days}
          className="input-field w-20 text-center py-1.5 text-sm"
          min={0}
          max={365}
        />
        <button
          onClick={() => onSave(type.id, value || type.max_days)}
          disabled={saving || !value}
          className="bg-kfs-green text-white text-xs px-3 py-1.5 rounded-lg
                     disabled:opacity-40 hover:bg-kfs-dark transition-colors"
        >
          Set
        </button>
      </div>
...
// ── Employee Card ────────────────────────────
function EmployeeCard({ employee, leaveTypes }) {
      ...
      <div className="card">
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <div className="w-10 h-10 rounded-full bg-kfs-green flex items-center
    ...
    <div className="page-container">
      ...
      {isLoading ? (
        ...
        <div className="card text-center py-12">
          ...
          <button
            onClick={() => setShowCreate(true)}
            className="btn-primary max-w-xs mx-auto mt-4 block"
          >
            Add First Employee
          </button>
        </div>
